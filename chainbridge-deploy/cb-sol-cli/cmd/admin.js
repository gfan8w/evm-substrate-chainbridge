...
const {setupParentArgs, safeSetupParentArgs, safeTransactionAppoveExecute, splitCommaList, waitForTx, log, logSafe, createResourceID,expandDecimals} = require("./utils")
...
const changeSpecialFeeCmd = new Command("set-specialFee")
    .description("Set a new special fee for deposits")
    .option('--bridge <address>', 'Bridge contract address', constants.BRIDGE_ADDRESS)
    .option('--specialFee <value>', 'New fee (in wei)', 0)
...
const safeChangeSpecialFeeCmd = new Command("safe-set-specialFee")
    .description("Set a new specialFee for deposits")
    .option('--bridge <address>', 'Bridge contract address', constants.BRIDGE_ADDRESS)
    .option('--specialFee <value>', 'New fee (in wei)', 0)
    .option('--specialFeeChainID <value>', 'New chainID', 0)
    .requiredOption('--multiSig <value>', 'Address of Multi-sig which acts as bridge admin')
    ...
    .action(async function (args) {
        ...
        logSafe(args, `Setting fee to ${args.fee} wei,  specialFeeChainID ${args.specialFeeChainID}`)

        await safeTransactionAppoveExecute(args, 'adminChangeSpecialFee',[args.specialFee, args.specialFeeChainID])
    })
...
const transferFunds = new Command("transfer-funds")
    ...
    .requiredOption('--amounts <value>', 'Array of amonuts to addrs', splitCommaList)
    .action(async function (args) {
        await setupParentArgs(args, args.parent.parent)
        const bridgeInstance = new ethers.Contract(args.bridge, constants.ContractABIs.Bridge.abi, args.wallet);
...
const generateResourceID = new Command("generate-resource-id")
    .description("generate resource id")
    .requiredOption('--address <address>', 'contract address, like: 0x9ea356d25c658A648f408ABE2322F2f01F12A0F0')
    .requiredOption('--chainId <value>', 'chain id where contract address belong to, like 1 in decimal')
    .action(async function (args) {
        await safeSetupParentArgs(args, args.parent.parent)

        log(args, `generate resource id with (${args.address}) and ${args.chainId}`)

        const resId=await createResourceID(args.address, args.chainId)

        console.log("ResourceID:",resId)
    })

const etcBalance = new Command("etc-balance")
    .description("get etc balance")
    .requiredOption('--address <address>', 'contract address, like: 0x9ea356d25c658A648f408ABE2322F2f01F12A0F0')
    .action(async function (args) {
        await safeSetupParentArgs(args, args.parent.parent)

        let balance = await args.provider.getBalance(args.address);

        console.log(`address:${args.address} has balance: ${ethers.utils.formatEther(balance)} eth`)
    })
...
adminCmd.addCommand(changeSpecialFeeCmd)
adminCmd.addCommand(safeChangeSpecialFeeCmd)
...
adminCmd.addCommand(generateResourceID)
adminCmd.addCommand(etcBalance)
