...
const transferCmd = new Command("transfer")
    .description("transfer erc20 tokens to other")
    .option('--address <value>', 'the target address that accept the erc20 tokens', '')
    .option('--amount <value>', 'Amount to transfer', 100)
    .option('--erc20Address <address>', 'ERC20 contract address', constants.ERC20_ADDRESS)
    .action(async function (args) {
        await setupParentArgs(args, args.parent.parent)
        ...
        const erc20Instance = new ethers.Contract(args.erc20Address, constants.ContractABIs.Erc20Mintable.abi, args.wallet);
        log(args, `Minting ${args.amount} tokens to ${args.wallet.address} on contract ${args.erc20Address}`);
        const tx = await erc20Instance.transfer(args.address, expandDecimals(args.amount, args.decimals));
        await waitForTx(args.provider, tx.hash)
    })
...
const increaseCmd = new Command("increase")
    .description("Increase the Approve tokens for transfer")
    .option('--amount <value>', "Amount to transfer", 1)
    ...
    .action(async function (args) {
        ...
        const erc20Instance = new ethers.Contract(args.erc20Address, constants.ContractABIs.Erc20Mintable.abi, args.wallet);
        log(args, `Approving ${args.spender} to increase the spending of ${args.amount} tokens on behalf of ${args.wallet.address}!`);
        const tx = await erc20Instance.increaseAllowance(args.spender, expandDecimals(args.amount, args.parent.decimals), { gasPrice: args.gasPrice, gasLimit: args.gasLimit});
        await waitForTx(args.provider, tx.hash)
    })
...
const depositETHCmd = new Command("depositETH")
    ...
    .action(async function (args) {
        ...
        // Make the deposit
        let tx = await bridgeInstance.depositETH(
            args.dest, // destination chain id
            args.resourceId,
            data,
            { gasPrice: args.gasPrice, gasLimit: args.gasLimit,value: expandDecimals(args.amount, args.parent.decimals).toHexString()}
        );
...
const allowanceCmd = new Command("allowance")
    ...
    .action(async function(args) {
        ...
        const decimals = await erc20Instance.decimals();
        log(args, `Spender ${args.spender} is allowed to spend ${ethers.utils.formatUnits(allowance, decimals)} tokens on behalf of ${args.owner}`)
    })
...
erc20Cmd.addCommand(increaseCmd)
